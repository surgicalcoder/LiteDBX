...
internal class IndexService(Snapshot snapshot, Collation collation, uint maxItemsCount)
{
    ...
    public IndexNode AddNode(CollectionIndex index, BsonValue key, PageAddress dataBlock, IndexNode last)
    {
        ...
        return AddNode(index, key, dataBlock, levels, last);
    }
        ...
        IndexNode last)
    {
        ...
        var leftNode = GetNode(index.Head);
        var counter = 0u;

        // scan from top left
        for (var currentLevel = MAX_LEVEL_LENGTH - 1; currentLevel >= 0; currentLevel--)
        {
            ...
            while (!right.IsEmpty && right != index.Tail)
            {
                ...
                var diff = rightNode.Key.CompareTo(key, Collation);
    ...
    public byte Flip()
    {
        ...
        for (var R = Randomizer.Next(); (R & 1) == 1; R >>= 1)
        {
            levels++;
    ...
    /// If not found but sibling = true and key are not found, returns next value index node (if order = Asc) or prev node (if
    /// order = Desc)
    /// </summary>
    public IndexNode Find(CollectionIndex index, BsonValue value, bool sibling, int order)
    {
        var leftNode = order == Query.Ascending ? GetNode(index.Head) : GetNode(index.Tail);
        var counter = 0u;

        for (var level = MAX_LEVEL_LENGTH - 1; level >= 0; level--)
        {
            ...
            while (!right.IsEmpty)
            {
                ...
                var diff = rightNode.Key.CompareTo(value, Collation);
