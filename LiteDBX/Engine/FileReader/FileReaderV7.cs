...
/// <summary>
/// Internal class to read old LiteDBX v4 database version (datafile v7 structure)
/// </summary>
internal class FileReaderV7 : IFileReader
{
    ...
    public void Open()
    {
        ...
        if (_settings.Password == null && !_header["salt"].AsBinary.IsFullZero())
        {
        ...
        _aes = _settings.Password == null ? null : new AesEncryption(_settings.Password, _header["salt"].AsBinary);
    }
    ...
    public IEnumerable<IndexInfo> GetIndexes(string collection)
    {
        ...
        {
            var name = Regex.Replace(index["name"].AsString, @"[^a-z0-9]", "", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        foreach (var indexPageID in indexPages)
        {
            ...
            foreach (var node in indexPage["nodes"].AsArray)
            {
                ...
                if (dataBlock["pageID"].AsInt32 != -1)
                {
                    ...
                    var data = block["extendPageID"] == -1 ? block["data"].AsBinary : ReadExtendData((uint)block["extendPageID"].AsInt32);
    ...
    /// <summary>
    /// Check header slots to test if data file is a LiteDBX FILE_VERSION = v7
    /// </summary>
